...
Source: `validate_cases()` in `pipeline/ciaboc_pipeline.py:371`.
...
| B. Appellate / writ calendar entries carry no CIABOC file number and no named suspect | 24 | Correct reject, but schema mismatch — these rows are unrepresentable. Open |
...
`pipeline/ciaboc_pipeline.py:369`:
...
Patch applied at `pipeline/ciaboc_pipeline.py:369`. Checked against 9 valid file
...
2. Add a regression test asserting `BC/C/1133/2016` and `BC/O/1203/2016` match and
   `SC/SPL/LA 182/2025` does not. The repo has no test suite yet, so this means
   standing one up.
3. Decide on option 1 vs 2 for the appellate rows and record the decision.
4. Delete the redundant `.bak` files, or add `*.bak` to `.gitignore` — right now
   `review_c67af4c7.json.bak` inflates the apparent backlog by duplicating seven rows.
   The same applies to the loose DB snapshots in `data/processed/`
   (`ciaboc.db.bak`, `.predatefix`, `.pre-regexfix`).
