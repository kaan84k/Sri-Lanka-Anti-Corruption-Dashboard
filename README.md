...
- **Automated ingestion** — a scraper pulls weekly CIABOC PDFs into an immutable `data/raw/` archive.
...
- **Identity resolution** — the same person under spelling or abbreviation variants is linked to a
  single canonical record through reviewed aliases.
- **No data loss** — rows that fail validation are written to a review queue, never silently dropped.
- **Read-only API** — FastAPI over SQLite with summary statistics, filtered case listings, and search.
- **Interactive dashboard** — React and Recharts: hearings over time, court-level composition,
  most-implicated institutions, repeat suspects, cross-filter drill-downs, and a light/dark theme.
...
- Python 3.11+
- Node.js 18+
- A Google **Gemini API key** (only required to *extract* new PDFs, not to run the API or dashboard)
...
This is an open-source civic-technology project, and contributions are welcome. Whether you write
Python, build React interfaces, care about data quality, or spotted a typo — there is a place for you.

### Ways to contribute
| Area | Examples |
|------|----------|
| **Pipeline** | improve extraction accuracy, add PDF-layout edge cases, more validation rules |
| **Dashboard** | new visualizations, accessibility, mobile polish, i18n (Sinhala / Tamil) |
...
| **Docs and design** | tutorials, screenshots, diagrams, this README |
| **Testing and CI** | unit tests, GitHub Actions, linting |

### How to get started
1. **Fork** the repository and create a branch: `git checkout -b feat/short-description`
2. Set up the component you are touching (see [Getting started](#getting-started)).
3. Make your change with clear, focused commits.
4. **Open a pull request** describing *what* changed and *why*. Screenshots are welcome for UI work.

> New here? Look for issues labeled **`good first issue`** — they are scoped to be approachable.
...
## License
No license has been chosen yet — until one is added, default copyright applies. If you would like to
help select an appropriate open-source license (for example MIT or Apache-2.0), please open an issue.
